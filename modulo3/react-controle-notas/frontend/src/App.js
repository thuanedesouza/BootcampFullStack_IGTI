...
const handleDelete = async (gradeToDelete) => {
  ...
  if (isDeleted){//pegando o id para colocar true em isDeleted desse elemento
    const deletedGradeIndex = allGrades.findIndex(grade => grade.id === gradeToDelete.id );
    
    const newGrades = Object.assign([], allGrades);
    //alteração interna para refletir a exclusão lógica
...
const handlePersistData = async (formData) => {
  ...
  // afinal handlepersistData vai ser invocado quando acontecer o onSave
};
